...
try {
    ...
    if ($action === 'approve') {
        ...
        $stmt = $pdo->prepare("
            ...
            (cpf, name, designation, mobile, section, subsection, ext, direct, dob, dor, level, class, seating_location, did_number, date_join_ongc, date_join_post, date_prom, date_join_area)
            VALUES 
            (:cpf, :name, :designation, :mobile, :section, :subsection, :ext, :direct, :dob, :dor, :level, :class, :seating_location, :did_number, :date_join_ongc, :date_join_post, :date_prom, :date_join_area)
        ");
        ...
        $stmt->execute([
            ...
            'date_join_ongc' => $request['date_join_ongc'],
            'date_join_post' => $request['date_join_post'],
            'date_prom' => $request['date_prom'],
            'date_join_area' => $request['date_join_area']
        ]);
    }
